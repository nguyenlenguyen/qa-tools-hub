...
// Random funny names for devices
const DEVICE_NAMES = [
...
export default function PeerFileShare() {
  ...
  const [deviceName, setDeviceName] = useState(() => {
    ...
    // Check if the saved name starts with one of our animal names
  ...
  const [peers, setPeers] = useState([]); // { id, name, type }
  const [connections, setConnections] = useState({}); // { peerId: dataConnection }
  const [transfers, setTransfers] = useState([]); // { id, role: 'send'|'receive', name, progress, status, file }
  ...
  const initDiscovery = async () => {
    try {
      ...
      // Generate a unique room topic based on IP hash (simple hash)
  ...
  const subscribeToTopic = (topic, myId) => {
    // We'll use ntfy.sh to announce ourselves and find others
    // Since we can't easily use WebSockets for free, ntfy is a great relay
    ...
    source.onmessage = (event) => {
      try {
        ...
        if (data.message) {
          ...
          // Ignore old or our own messages
          const messageTime = payload.timestamp || 0;
          const now = Date.now();
          if (now - messageTime > 30000) return; // Ignore messages older than 30s
          
          if (payload.type === 'presence' && payload.peerId !== myId) {
            // New peer found!
            addPeer({
              id: payload.peerId,
            ...
            // Send back our presence to the newcomer
            if (payload.isNew) {
              sendPresence(topic, myId, deviceName, false);
            }
          }
        }
      } catch (err) {
  ...
  useEffect(() => {
    // 1. Heartbeat to stay visible
    const heartbeat = setInterval(() => {
      if (roomTopic && peerId && status === 'ready') {
        sendPresence(roomTopic, peerId, deviceName, false);
      }
    }, 10000);
  ...
  }, [roomTopic, peerId, deviceName, status]);
  ...
  const setupConnection = (conn) => {
    ...
    conn.on('data', (data) => {
      if (data.type === 'file-meta') {
        ...
        // Handling chunks (simplified: we'll just send the whole blob for now as PeerJS does it well)
        // But for UI progress, chunks are better. PeerJS sends blobs effectively.
      } else if (data instanceof Blob || data instanceof ArrayBuffer) {
        // PeerJS auto-serializes blobs. If it is a blob, it's the file content.
        // We'll match it with the last registered transfer from this peer
        setTransfers(prev => {
          const updated = [...prev];
  ...
  const sendFile = (targetPeerId, file) => {
    ...
    const sendAfterOpen = () => {
      ...
      // PeerJS handles large blobs by chunking them internally in many versions
      conn.send(file);
  ...
  const filteredPeers = peers.filter(peer => peer.name !== deviceName);
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                      <div 
                        className={`h-full transition-all duration-300 ${t.status === 'complete' ? 'bg-emerald-500' : 'bg-blue-500'}`}
